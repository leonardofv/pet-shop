...
export const DatePicker = () => {
  ...
  const getInitialDate = useCallback(() => {
    ...
    const parseDate = new Date(year, month - 1, day); //janeiro mês 0

    if (!isValid(parseDate)) return new Date();
  ...
  const updateUrlWithDate = (selectedDate: Date | undefined) => {
    ...
    const newParams = new URLSearchParams(searchParams.toString()); //criar url
    newParams.set('date', format(selectedDate, 'yyyy-MM-dd')); //setar a data
    router.push(`${pathName}?${newParams.toString()}`); //atualizar rota
  };
